...
void FixedMemoryResource::do_deallocate(void* ptr, size_t bytes, size_t /*alignment*/) {
    ...
    // Ключевой момент: память НЕ возвращается в ОС, а остаётся в нашем пуле
...
void* FixedMemoryResource::find_free_block(size_t bytes, size_t alignment) {
    // Используем lower_bound для эффективного поиска первого блока >= bytes
    // Это работает за O(log n) благодаря упорядоченности map по размеру
